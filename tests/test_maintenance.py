...
@pytest.mark.anyio
async def test_preview_and_apply_check_database(tmp_path: Path) -> None:
    ...
    assert preview["problems_truncated"] is False
...
@pytest.mark.anyio
async def test_preview_and_apply_optimize_database(tmp_path: Path) -> None:
    path = tmp_path / "collection.anki2"
    _seed(path, empty=0, valid=1)

    async with AnkiCollectionService(str(path), max_page_size=100) as service:
        counts = await service.executor.run(
            lambda adapter: (adapter.collection.card_count(), adapter.collection.note_count())
        )
    ...
    assert preview["card_count"] == counts[0]
    assert preview["note_count"] == counts[1]
