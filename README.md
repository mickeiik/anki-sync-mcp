...
review history and analytics, backups, and explicit full-sync recovery. Tag tools cover listing
(optionally with note counts that include child tags), renaming, deleting, and merging a tag and
its children into another tag.
...
| Variable | Default | Description |
| --- | --- | --- |
| `ANKI_ALLOW_DESTRUCTIVE` | `false` | Registers delete, deck-preset deletion, tag-merge, FSRS rescheduling, permanent media-trash emptying, and collection-maintenance (check database, empty cards, optimize) preview/apply tools when `MCP_SCOPES` also includes `destructive`. Applying destructive changes requires a preview token and verified fresh backup. The verified backup covers the collection database only, so emptied media cannot be restored. |
...
| `ANKI_ALLOW_SCHEMA_CHANGES` | `false` | Registers note-type and template mutation tools when `ANKI_ALLOW_FULL_SYNC` is also enabled. Note-type deletion and deck-preset deletion additionally require destructive access. |
| `ANKI_ALLOW_REVIEW_ANSWERS` | `false` | Registers the administrative card-answering tool, which changes scheduling state. |
| `ANKI_ALLOW_UNDO` | `false` | Registers the `anki_undo` and `anki_redo` tools when `MCP_SCOPES` also includes `destructive` and `ANKI_SYNC_ON_WRITE=false`. Undo history is in-memory, is cleared by a successful synchronization, and is not durable across restarts, so with the default `ANKI_SYNC_ON_WRITE=true` (where every mutation syncs) the two tools are not registered at all. `anki_undo_status` stays available and reports a `disabled_reason` naming every blocking setting. |
| `ANKI_ALLOW_IMPORT` | `false` | Registers the destructive-scope `.apkg`/`.csv` import tools when `MCP_SCOPES` also includes `destructive`. Files must be placed in the collection's `imports/` folder (for the container: `/data/imports/`, e.g. with `docker cp`) or passed inline as `content_base64` (staged under a content-derived name in that folder; capped by `ANKI_MAX_IMPORT_BYTES` and by the MCP request-body limit). Staged files are size-capped by `ANKI_MAX_IMPORT_BYTES`, and every import requires a preview token plus a verified pre-operation backup. `.apkg` import additionally requires `ANKI_ALLOW_SCHEMA_CHANGES` and `ANKI_ALLOW_FULL_SYNC`, because packages can add note types and change scheduling. The CSV preview token binds the file identity (content digest) and delimiter, while the note type, field mapping, deck, tags, and duplicate resolution are chosen at apply time by the same authenticated caller. |
...
| `ANKI_MAX_IMPORT_BYTES` | `268435456` | Caps both the staged file size and, for an `.apkg`, the declared uncompressed size of its members (`1048576`-`4294967296` bytes); files over the cap are rejected before parsing or inflation. |
