...
| `ANKI_ALLOW_UNDO` | `false` | Registers the `anki_undo` and `anki_redo` tools when `MCP_SCOPES` also includes `destructive`. Undo history is in-memory, is cleared by synchronization, and is not durable across restarts. With the default `ANKI_SYNC_ON_WRITE=true` every mutation syncs and therefore clears the history, so undo/redo only work when synchronization is disabled (`ANKI_SYNC_ON_WRITE=false`). |
