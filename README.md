...
| `ANKI_ALLOW_UNDO` | `false` | Registers the `anki_undo` and `anki_redo` tools when `MCP_SCOPES` also includes `destructive` and `ANKI_SYNC_ON_WRITE=false`. Undo history is in-memory, is cleared by synchronization, and is not durable across restarts, so with the default `ANKI_SYNC_ON_WRITE=true` (where every mutation syncs) the two tools are not registered at all. `anki_undo_status` stays available and reports a `disabled_reason`. |
