...
| Variable | Default | Description |
| --- | --- | --- |
| `ANKI_ALLOW_DESTRUCTIVE` | `false` | Registers delete, tag-merge, FSRS rescheduling, and permanent media-trash emptying preview/apply tools when `MCP_SCOPES` also includes `destructive`. Applying destructive changes requires a preview token and verified fresh backup. The verified backup covers the collection database only, so emptied media cannot be restored. |
